...
Use REST for reads and writes and SSE for authenticated trade-update notifications. Define HTTP payloads with backend Zod schemas, publish OpenAPI, and generate frontend types, clients, hooks, and validators with Orval.

## Rationale

Trade updates are server-to-client notifications, so SSE is sufficient and operationally smaller than WebSockets. REST remains authoritative. If bidirectional streaming becomes necessary, the notification transport can later be replaced without changing write semantics. Generated contracts prevent frontend/backend contract drift.
